  ...
  <div class="profileMain" v-show="readyShow">
    ...
    >
      <template v-slot:body-cell-disable="item">
        <td class="text-left">
          <q-toggle :disable="store.state.whoAmi.login===item.row.login" :model-value="item.value" @update:model-value="(state)=>toggleDisableUser(item,state)" />
        </td>
      </template>
      <template v-slot:body-cell-group="item">
        <td class="text-left">
          <q-toggle
            :disable="store.state.whoAmi.login===item.row.login"
    ...
    <q-btn v-if="store.state.whoAmi.group!=='admin'" class="changePrivateBtn controlButtonSize" color="grey-10" label="Edit user data" @click="registerShow=true" />
...
async function requestUserData () {
  let srcUserInfo
  if (store.state.whoAmi.group === 'admin') {
    ...
    if (!result?.meta) {
      ...
      srcUserInfo = userList.value.find(({ login }) => login === store.state.whoAmi.login)
    }
  } else {
